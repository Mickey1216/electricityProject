            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')"> 
                  <a>综合<i v-show="isOne" class="iconfont" :class="{'icon-jiantou_xiangxia_o':isDesc,'icon-jiantou_xiangshang_o':isAsc}"></i></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<i v-show="isTwo" class="iconfont" :class="{'icon-jiantou_xiangxia_o':isDesc,'icon-jiantou_xiangshang_o':isAsc}"></i></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //删除分类的名字
    removeCategoryName(){
      //把带给服务器的参数置空，还需要向服务器发请求；带给服务器参数是可有可无的：
      //属性值为空字符串的字段还是会把相应的字段带给服务器；但是如果属性值为undefined，则当前这个字段不会带给服务器。
    ...
    //排序操作回调函数
    changeOrder(flag){  //flag是一个标记，代表用户点击的是综合（1）还是价格（2）【用户点击的时候传递进来的】
      //这里获取的是最开始的状态
      ...
      //准备一个新的order值
      let newOrder = ''
      if(flag === originFlag){ //判断多次点击的是不是同一个按钮
  ...
  computed: {
    //mapGetters写法：传递数组，因为getters计算是没有划分模块
  ...
  watch:{
    ...
    $route(newValue,oldValue){
      ...
      //再次发请求
      this.getData()
      //每一次请求完毕后，应该把相应的1，2，3级分类id置空，让它重新接收下一次相应的1，2，3级id
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    }
